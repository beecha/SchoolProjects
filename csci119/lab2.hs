...
cat = (distinct .) . zipWith (++)

starn :: Int -> Language -> Language
starn 0 _  = one
starn n as = ((`uni` astar) . (`cat` astar)) as
  where
    astar = starn (n-1) as
...
-- star, with n=5.
-- lang_of (reparse "a+b") = ["a"]
lang_of :: RE -> Language
lang_of Empty = []
lang_of (Letter ch) = ch
-- lang_of (Union r1 r2) = Union (lang_of r1) (lang_of r2) 
...
nepart (Cat r1 r2) | has_epsilon r1 && has_epsilon r2 = Cat (nepart r1) (nepart r2)
                   | otherwise = (Cat r1 r2)
nepart (Star r) = r `Cat` (Star r)
