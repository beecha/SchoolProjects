...
match_correct r [] = True
match_correct Empty s = (s == "")
match_correct (Letter c) s = (s == [c])
match_correct (Union r1 r2) s = match_correct r1 s || match_correct r2 s
match_correct (Cat r1 r2) s = or [match_correct r1 s1 && match_correct r2 s2 | (s1,s2) <- splits s]
match_correct (Star r) s = match_correct Empty s || or [match_correct r s1 && match_correct (Star r) s2 | (s1,s2) <- splits s]
-- match_correct r s = undefined

match :: Matcher
match Empty st = (st == "")
match (Letter c) st = (st == [c])
match (Union r1 r2) st = match r1 st || match r2 st
match (Cat r1 r2) st = or [match r1 s1 && match r2 s2 | (s1,s2)<-splits st]
